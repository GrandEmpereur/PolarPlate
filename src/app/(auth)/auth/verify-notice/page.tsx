...
// Composant contenant la logique qui utilise useSearchParams
...
function VerifyNoticeSkeleton() {
  return (
    <Card className="w-full max-w-md border-border/40 shadow-lg">
      ...
      <CardContent className="space-y-4">
        <div className="bg-muted p-4 rounded-md animate-pulse h-32"></div>
        <div className="h-10 rounded-md animate-pulse bg-muted"></div>
      </CardContent>
      <CardFooter className="flex-col space-y-4 border-t border-border/40 bg-muted/20 p-6">
        <div className="text-center text-sm">
          <span className="inline-flex items-center text-muted-foreground">
            <ArrowLeft className="mr-1 h-3 w-3" /> Chargement...
          </span>
        </div>
      </CardFooter>
    </Card>
...
export default function VerifyNotice() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-background to-muted/20 p-4">
      <Suspense fallback={<VerifyNoticeSkeleton />}>
        <VerifyNoticeContent />
      </Suspense>
    </div>
  );
